...
function buildSlotsFor(chargerId, dateIso) {
  ...
  // Deterministic pseudo-random "taken" pattern per charger+date
...
assert old1 in content, "BLOCK 1 NOT FOUND"
...
old2 = '''function App() {
  ...
  function handleConfirm() {
    ...
    // reset selection after confirm (mock)
...
new3 = '''        <section className="station-head reveal" id="station-header" data-station-id={station ? station.id : ''}>
          <span className="eyebrow">Station detail · {station ? station.id : '...'}</span>
          <h1 className="title">
            {station ? (
              <>
          ...
          <div className="head-meta">
            <span className="badge badge-dot badge-active">Open now</span>
            <span className="badge">{chargers.length} chargers</span>
...
# ── 5. Booking panel charger hint (template literal) ──
old5 = "                {selectedCharger ? `Charger \xc2\xb7 ${selectedCharger}` : 'Select charger first'}"
new5 = "                {chargerObj ? 'Charger \xc2\xb7 ' + chargerObj.typeName : 'Select charger first'}"
if old5 not in content:
    # try with unicode dot directly
    old5 = "                {selectedCharger ? `Charger · ${selectedCharger}` : 'Select charger first'}"
    new5 = "                {chargerObj ? 'Charger · ' + chargerObj.typeName : 'Select charger first'}"
assert old5 in content, "BLOCK 5 NOT FOUND: " + repr(content[content.find('Select charger first')-60:content.find('Select charger first')+30])
...
new6 = '''                    {loadingSlots ? 'Loading...' : selectedCharger
                      ? <>{slots.filter(s=>!s.taken).length} <em>open</em> · {slots.filter(s=>s.taken).length} taken</>
                      : 'Pick a charger'}'''
assert old6 in content, "BLOCK 6 NOT FOUND"
...
print("Station Booking.html patched OK")
